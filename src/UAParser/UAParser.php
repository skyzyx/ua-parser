 ...
 *
 * UAParser is the PHP library for the ua-parser project. Learn more about the ua-parser project at:
 *
...
class UAParser
{
	 ...
	 *
	 * @param  string   $ua User agent string to test. The default value is an empty string.
	 * @return stdClass     The result of the user agent parsing.
	 */
	 ...
	 *
	 * @param  string   $uaString User agent string to test.
	 * @return stdClass           The result of the user agent parsing.
	 */
		...
		foreach ($uaRegexes as $uaRegex)
		{
			...
			if (preg_match('/' . str_replace('/', '\/', str_replace('\/', '/', $uaRegex->regex)) . '/i', $uaString, $matches))
			{
				...
				$ua->family = isset($uaRegex->family_replacement) ?
					str_replace('$1', $matches[1], $uaRegex->family_replacement) :
					$matches[1];

				// version properties
				$ua->major = isset($uaRegex->v1_replacement) ?
					$uaRegex->v1_replacement :
					$matches[2];

				$ua->minor = isset($uaRegex->v2_replacement) ?
					$uaRegex->v2_replacement :
					$matches[3];

				$ua->patch = isset($uaRegex->v3_replacement) ?
					$uaRegex->v3_replacement :
					$matches[4];
	 ...
	 *
	 * @param  string   $uaString User agent string to test.
	 * @return stdClass           The result of the OS parsing.
	 */
	public function osParse($uaString = '')
	{
		...
		foreach ($osRegexes as $osRegex)
		{
			if (preg_match('/' . str_replace('/', '\/', str_replace('\/', '/', $osRegex->regex)) . '/i', $uaString, $matches))
			{
				...
				// os name
				$os->family = isset($osRegex->os_replacement) ?
					$osRegex->os_replacement :
					$matches[1];

				// version properties
				$os->major = isset($osRegex->os_v1_replacement) ?
					$osRegex->os_v1_replacement :
					$matches[2];

				$os->minor = isset($osRegex->os_v2_replacement) ?
					$osRegex->os_v2_replacement :
					$matches[3];

				$os->patch = isset($osRegex->os_v3_replacement) ?
					$osRegex->os_v3_replacement :
					$matches[4];

				$os->patch_minor = isset($osRegex->os_v4_replacement) ?
					$osRegex->os_v4_replacement :
					$matches[5];
	 ...
	 *
	 * @param  string   $uaString User agent string to test.
	 * @return stdClass           The result of the device parsing.
	 */
	public function deviceParse($uaString = '')
	{
		...
		foreach ($deviceRegexes as $deviceRegex)
		{
			if (preg_match('/' . str_replace('/', '\/', str_replace('\/', '/', $deviceRegex->regex)) . '/i', $uaString, $matches))
			{
				...
				// device name
				$device->family = isset($deviceRegex->device_replacement) ?
					str_replace('$1', str_replace("_", " ", $matches[1]), $deviceRegex->device_replacement) :
	 ...
	 *
	 * @param  stdClass $obj The object (`ua` or `os`) to be used.
	...
	public function toString($obj)
	{
		...
		$string = !empty($versionString) ?
			$obj->family . ' ' . $versionString :
			$obj->family;
	 ...
	 *
	 * @param  stdClass $obj The obj that contains version number bits.
	 * @return string        The result of combining the version number bits together.
	 */
	public function toVersionString($obj)
	{
		$versionString = isset($obj->major) ?
			$obj->major :
			'';

		$versionString = isset($obj->minor) ?
			$versionString . '.' . $obj->minor :
			$versionString;

		$versionString = isset($obj->patch) ?
			$versionString . '.' . $obj->patch :
			$versionString;

		$versionString = isset($obj->patch_minor) ?
			$versionString . '.' . $obj->patch_minor :
			$versionString;
	...
	/**
	 * Returns a string consisting of the family and full version number for both the browser and os.
	 *
	 * @param  stdClass $ua The `ua` object.
	 * @param  stdClass $os The `os` object.
	 * @return string       The result of combining family and version.
	 */
	 ...
	 *
	 * @param  stdClass $data The data to log.
	 * @return void
	 */
